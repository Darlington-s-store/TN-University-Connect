...
export default function AdminSettings() {
  const [settings, setSettings] = useState({
    ...
    description: "Uniting Ghana's universities, students, and alumni through one connected platform.",
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <Card>
        <CardContent className="p-6 space-y-5">
          ...
          <div><Label>Description</Label><Textarea rows={3} value={settings.description} onChange={(e) => setSettings({ ...settings, description: e.target.value })} /></div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          ...
          {[
            { key: "allowRegistration", label: "Allow new member registrations", desc: "When off, the register page is disabled." },
            { key: "emailNotifications", label: "Email notifications", desc: "Send notification emails on key events." },
            { key: "maintenance", label: "Maintenance mode", desc: "Show a maintenance banner to all visitors." },
          ].map((s, i) => (
            <div key={s.key}>
              ...
              <div className="flex justify-between items-center">
                ...
                <Switch checked={(settings as any)[s.key]} onCheckedChange={(v) => setSettings({ ...settings, [s.key]: v } as any)} />
              </div>
            </div>
          ))}
        </CardContent>
