...
export default function AdminBlog() {
  ...
  const persist = (next: BlogPost[]) => { saveBlogs(next); setList(next); };

  const blank = (): BlogPost => ({
    id: `b-${Date.now()}`,
    title: "", category: "General", author: "", date: new Date().toISOString().slice(0, 10),
    excerpt: "", body: "", published: true, featured: false,
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-start gap-3">
        <div>
          ...
          <p className="text-muted-foreground">Write, edit and publish stories for the TNUC blog.</p>
        </div>
        <Button onClick={() => { setEditing(blank()); setOpen(true); }}><Plus className="h-4 w-4" /> New post</Button>
      </div>
            ...
            <CardContent className="p-5">
              <div className="flex justify-between items-start gap-2 mb-2">
                ...
                <div className="flex gap-1">
                  {b.featured && <Badge className="bg-accent text-accent-foreground"><Star className="h-3 w-3" /></Badge>}
                  {b.published ? <Badge className="bg-primary">Live</Badge> : <Badge variant="outline">Draft</Badge>}
                </div>
              </div>
              ...
              <div className="text-xs text-muted-foreground mt-2">{b.author} · {new Date(b.date).toLocaleDateString("en-GB")}</div>
              <div className="flex gap-1 mt-4 pt-3 border-t">
                <Button size="sm" variant="ghost" onClick={() => { persist(list.map((x) => x.id === b.id ? { ...x, published: !x.published } : x)); }}>
                  {b.published ? <><EyeOff className="h-4 w-4" /> Unpublish</> : <><Eye className="h-4 w-4" /> Publish</>}
                </Button>
                <Button size="sm" variant="ghost" onClick={() => { setEditing({ ...b }); setOpen(true); }}><Pencil className="h-4 w-4" /> Edit</Button>
                <Button size="sm" variant="ghost" onClick={() => { persist(list.filter((x) => x.id !== b.id)); toast.success("Deleted"); }}><Trash2 className="h-4 w-4 text-destructive" /></Button>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader><DialogTitle>Blog post</DialogTitle></DialogHeader>
          {editing && (
