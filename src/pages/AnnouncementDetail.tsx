  ...
  return (
    <article className="py-16">
      <div className="container mx-auto px-4 sm:px-6 max-w-3xl">
        <Button asChild variant="ghost" className="mb-6"><Link to="/announcements"><ArrowLeft className="h-4 w-4" /> All announcements</Link></Button>
        ...
        <div className="flex items-center gap-4 text-sm text-muted-foreground mb-8 pb-8 border-b">
          <span className="flex items-center gap-1"><Calendar className="h-4 w-4" /> {new Date(item.date).toLocaleDateString("en-GB", { dateStyle: "long" })}</span>
          <button className="flex items-center gap-1 hover:text-primary"><Share2 className="h-4 w-4" /> Share</button>
        </div>
        <div className="prose prose-lg max-w-none">
          <p className="text-lg leading-relaxed text-foreground/90 whitespace-pre-wrap">{item.body}</p>
        </div>
      </div>
    </article>
  );
